...
def val(args, vis, device, val_loader, seg_model, train_model, criterion, epoch):
    epoch_loss = []       
    ...
    train_model.eval()  # train_model in eval mode for validation

    with torch.no_grad():
        for image_lst, SDF_image_lst, label_lst, image_name_lst in val_loader:
            ...
            if epoch <= args.switch_epoch:
                # Stage 1: Only use original image for validation (first args.switch_epoch epochs)
            ...
            # Calculate loss
            loss = 0.5 * criterion["DiceLoss"](final_pred, label) + 0.5 * criterion["BCEloss"](final_pred, label)
        ...
        print("================================ Epoch:{} Valid Metric =====================================".format(epoch))
