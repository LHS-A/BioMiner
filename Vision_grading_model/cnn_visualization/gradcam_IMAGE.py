...
def process_dataset(data_root, model):
    """
    Process the dataset with single image directory
    :param data_root: Root directory of the dataset
    :param model: Pre-trained model
    """
    img_dir = os.path.join(data_root, "image")
    seg_dir = os.path.join(data_root, "Predict_label")
    roi_output_dir = os.path.join(data_root, "ROI_image")
    cam_output_dir = os.path.join(data_root, "CAM_image")
...
def main():
    ...
    # Process the dataset
    print(f"\n{'='*50}")
    process_dataset(DATA_PATH, model)
    print(f"Completed processing dataset")
    
    print(f"\n{'='*50}")
    print("All ROI images and CAM heatmaps generated successfully!")
    print(f"ROI images saved in: {os.path.join(DATA_PATH, 'ROI_image')}")
    print(f"CAM heatmaps saved in: {os.path.join(DATA_PATH, 'CAM_image')}")
